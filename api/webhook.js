...
async function processMessage({ message, phoneNumberId }) {
  try {
    ...
    extractOrder(text, history, systemPrompt)
      .then((order) => order && createOrder({ phone: from, phoneNumberId, items: order.items, amount: order.amount }))
      .catch((e) => console.error("Order extraction/creation failed:", e));

    try {
      await sendWhatsAppText(phoneNumberId, from, reply);
    } catch (sendErr) {
      // FIX: this used to be unguarded — any failure here (bad "to",
      // 131030 test-mode recipient restriction, transient Meta API
      // error, etc.) bubbled up to the outer catch and the customer
      // got nothing at all, with no distinct log signal to search for.
      console.error("Final WhatsApp send failed for", from, ":", sendErr);
      return;
    }

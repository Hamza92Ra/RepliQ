...
export async function setConversationOwner(phone, phoneNumberId) {
    ...
    await redisPipeline([["SET", `conv:owner:${phone}`, phoneNumberId]]);
}
...
export async function deleteConversation(phone) {
    ...
    if (!normalizedPhone) {
        throw new Error("Invalid conversation phone number");
    }

    await redisPipeline([
        ["DEL", `conv:${normalizedPhone}`, `conv:mode:${normalizedPhone}`, `conv:owner:${normalizedPhone}`],
        ["SREM", "conv:index", normalizedPhone],
    ]);
}
